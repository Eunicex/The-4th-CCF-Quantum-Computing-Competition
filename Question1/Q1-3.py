...
from pyqpanda import QCircuit, CNOT, SWAP
...
if __name__ == "__main__":
    ...
    """
    # 1. 构建三个CNOT门实现的SWAP电路
    circuit_cnot_swap = QCircuit()
    circuit_cnot_swap << CNOT(qbits[0], qbits[1])
    circuit_cnot_swap << CNOT(qbits[1], qbits[0])
    circuit_cnot_swap << CNOT(qbits[0], qbits[1])
    ...
    matrix_cnot_swap = np.matrix(get_matrix(circuit_cnot_swap, False)).reshape((4, 4))
